...
const BASE = 'c:/Users/rgama/OneDrive/Desktop/Aplicativo de academia';
...
const erros = [];
const valores = [];
const stats = { porGrupo: {}, porPadrao: {}, porComum: {}, semIncremento: 0, unilaterais: 0 };

for (const [i, linha] of linhas.entries()) {
  const c = linha.split(',');
  if (c.length !== 10) { erros.push(`linha ${i + 2}: ${c.length} colunas`); continue; }

  const [id, nome, grupo, secundarios, padrao, equip, incr, uni, medida, comum] = c;
  ...
  valores.push(
    ...
    `${incr.trim() ? incr : 'null'}, ${uni.toLowerCase() === 'true'}, ${q(medida)}, ${comum})`
  );
